--!nocheck
-- Credits to Nexure! https://github.com/Nexure/roact-flow/tree/feature/group-animations/src/Utility
...
function SpringValue:Run(onFinish: () -> ()?, update: () -> ()?)
	if update then
		self._updater = update
        task.spawn(update, self:GetValue())
	end

    SpringValues[self] = onFinish or function() end
...
RunService.PreRender:Connect(function(dt: number)
	for spring, resolve in pairs(SpringValues) do
		...
		if not didUpdate then
			...
			if spring._updater then
				spring._updater(spring:GetGoal())
			end
